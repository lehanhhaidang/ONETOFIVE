...
<body> 
    ...
    ?>
        <section>
   <main>
    <h1 style="text-align: center">PHIẾU XUẤT NGUYÊN VẬT LIỆU</h1>
   			 <?php
			   if (isset($_REQUEST['maPXNVL']))
					{
						$maPXNVL = $_REQUEST['maPXNVL'];
					}
						 $p->xuatCTPhieuXNVL("SELECT *
										FROM phieuxnvl
										INNER JOIN kho ON phieuxnvl.maKho = kho.maKho
										INNER JOIN nhanvienkho ON kho.maNVKho = nhanvienkho.maNhanVien
										WHERE phieuxnvl.maPXNVL = '$maPXNVL'");
			?>
					   ...
					   if (isset($_REQUEST['maPXNVL']))
							{
								...
								INNER JOIN phieuxnvl ON longuyenvatlieu.maPXNVL = phieuxnvl.maPXNVL WHERE phieuxnvl.maPXNVL = '$maPXNVL'
									");
					?>
                </tbody>
            </table>
