...
<head>
    ...
    <script>
      function cancelAction() {
          ...
          window.location = 'PhieuNhapNVL.php';
      }
    </script>
    <title>Lập Biên Bản Bồi Thường NVL</title>
</head>
<body> 
    <header>
    <?php
   ...
   <main>
    ...
    <h1> Lập Biên Bản Bồi Thường Nguyên Vật Liệu </h1>
   			 <?php
			   if (isset($_REQUEST['maPNNVL']))
					{
						$maPNNVL = $_REQUEST['maPNNVL'];
						 $p->lapTrenLBBBTNVL("SELECT pnnvl.maPNNVL, pnnvl.tenNguoiGiao, kho.tenKho
                         FROM phieunnvl pnnvl
                         JOIN kho ON pnnvl.maKho = kho.maKho
                         WHERE pnnvl.maPNNVL = '$maPNNVL';
					   ...
					   if (isset($_REQUEST['maPNNVL']))
							{
								$maPNNVL = $_REQUEST['maPNNVL'];
                                 ...
                                 KE.`tenKe`
                             FROM
                                 `longuyenvatlieu` AS LNVL
                             JOIN
                                 `nguyenvatlieu` AS NVL ON LNVL.`maNguyenVatLieu` = NVL.`maNguyenVatLieu`
                             JOIN
                                 `ke` AS KE ON LNVL.`maKe` = KE.`maKe`
                             JOIN
                                 `phieunnvl` AS PNNVL ON LNVL.`maPNNVL` = PNNVL.`maPNNVL`
                             WHERE
                                 PNNVL.`maPNNVL` = '$maPNNVL';
              ...
              if(isset($_REQUEST["btnlapphieu"])){
                ...
                if($dembt == 0){
                    ...
                    // Kiểm tra xem ghi chú có được nhập không
                        ...
                        echo "<script> window.location='PhieuNhapNVL.php'</script>";
                    }elseif($kq == 0){
